...
function Attack() {
    ...
    useEffect(() => {
        ...
        // Retrieve land numbers from localStorage
        const storedLandNo = localStorage.getItem("attackLandNo");
        const storedTargetLandNo = localStorage.getItem("attackTargetLandNo");
        
        if (storedLandNo) {
            setLandNo(storedLandNo);
        }
        if (storedTargetLandNo) {
            setTargetLandNo(storedTargetLandNo);
        }
    ...
    // Save land numbers to localStorage when they change
    useEffect(() => {
        if (landNo !== "") {
            localStorage.setItem("attackLandNo", landNo);
        }
    }, [landNo]);

    useEffect(() => {
        if (targetLandNo !== "") {
            localStorage.setItem("attackTargetLandNo", targetLandNo);
        }
    }, [targetLandNo]);
    ...
    const handleAttack = async (e) => {
        ...
        const sourceLand = Number(landNo);
        const targetLand = Number(targetLandNo);
        
        if (!sourceLand || sourceLand < 1 || sourceLand > 33) {
            setError('Your land number must be between 1 and 33');
            return;
        }
        
        if (!targetLand || targetLand < 1 || targetLand > 33) {
            setError('Target land number must be between 1 and 33');
            return;
        }
        
        if (sourceLand === targetLand) {
            setError('You cannot attack your own land');
            return;
        }
            ...
            {isMobileMenuOpen && (
                <div className="fixed top-16 left-0 w-full bg-white shadow-lg z-40 lg:hidden border-b border-blue-200">
                    <div className="flex flex-col py-4 justify-center items-center">
                        ...
                        <button className="flex items-center px-6 py-3 text-blue-700 bg-blue-50 transition"><LuSwords className="w-5 h-5 mr-3" /><span className="font-semibold">Attack</span></button>
                ...
                <div className={`transform transition-all duration-1000 ease-out w-full max-w-2xl ${
                    ...
                    <div className="bg-white/90 backdrop-blur-md rounded-2xl sm:rounded-3xl shadow-2xl p-4 sm:p-6 lg:p-8 border border-white/30 relative w-full">
                        ...
                        <div className="relative z-10 flex flex-col items-center w-full">
                            <form className="w-full max-w-xl flex flex-col gap-4" onSubmit={handleAttack}>
                                {/* Attack Land Number */}
                                <div className="flex flex-col items-center gap-4 w-full">
                                    <label htmlFor="attack-land-label" className="block bg-gray-700 font-bold text-lg sm:text-2xl mb-2 whitespace-nowrap text-white px-1 py-2 rounded-lg shadow-sm text-center">Your Land Number</label>
                                    <input
                                ...
                                <div className="flex flex-col items-center gap-4 w-full">
                                    <label htmlFor="attack-target-label" className="block bg-gray-700 font-bold text-lg sm:text-2xl mb-2 whitespace-nowrap text-white px-1 py-2 rounded-lg shadow-sm text-center">Land you want to attack</label>
                                    <div className="flex flex-col sm:flex-row gap-2 w-full max-w-md">
                                        <input
                                            id="attack-target-label"
                                            type="text"
                                            ...
                                            placeholder="must be from 1 - 33"
                                            value={targetLandNo}
                                            onChange={(e) => setTargetLandNo(e.target.value)}
                                        />
                                        <button
                                            type="submit"
                        ...
                        <div className="flex flex-col items-center mt-6 sm:mt-8">
                            <img src={image} alt="Attack Visual" className="w-full max-w-[700px] h-auto sm:h-[400px] object-cover rounded-xl shadow-lg" />
                        </div>
                    </div>
                </div>
                <ToastContainer />
